...
Target Markdown is a powerful interface to the `targets` R package for reproducible analysis pipelines. In interactive mode (the green arrow to the right-hand side of the chunk in RStudio) specialized code chunks test and prototype your pipeline without modifying the `_targets` data store. In non-interactive mode (the Knit button in RStudio) these code chunks write scripts to define your pipeline, allowing you to work entirely inside a single R Markdown file.
