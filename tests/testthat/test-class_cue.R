tar_test("cue_record()", {
  local <- algorithm_init(
    "local",
    pipeline_init(list(target_init("x", quote(1))))
  )
  local$run()
...
tar_test("cue_always()", {
  for (index in seq_len(2)) {
    ...
    local <- algorithm_init("local", pipeline_init(list(target)))
    local$run()
    out <- counter_get_names(local$scheduler$progress$built)
    expect_equal(out, "x")
  }
...
tar_test("cue_never()", {
  local <- algorithm_init(
    "local",
    pipeline_init(list(target_init("x", quote(1L))))
  )
  local$run()
...
tar_test("cue_command()", {
  ...
  local <- algorithm_init(
    "local",
    pipeline_init(list(target_init("x", quote(2L))))
  )
  local$run()
  out <- counter_get_names(local$scheduler$progress$built)
  expect_equal(out, "x")
...
tar_test("cue_depend()", {
  ...
  local <- algorithm_init("local", pipeline_init(list(x, y, z)))
  local$run()
  out <- counter_get_names(local$scheduler$progress$built)
