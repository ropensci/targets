...
#' @export
#' @family targets
#' @description Define a custom target storage format
#'   for `tar_target(format = "store_custom")`.
#' @return Object of class `"tar_resources_store_custom"`, to be supplied
#'   to the `store_custom` argument of `tar_resources()`.
#' @section Marshalling:
...
#'     custom = tar_resources_store_custom(
#'       read = function(path) {
...
#'       },
#'       repository = "none" # Could be "aws" (equivalent to format = "aws_keras")
#'     )
#'   )
#' )
tar_resources_store_custom <- function(
  read = function(path) {
    readRDS(path) # nocov
  },
  write = function(object, path) {
  ...
  out <- resources_store_custom_init(
    read = read,
    write = write,
    marshal = marshal,
    unmarshal = unmarshal,
