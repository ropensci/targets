...
* `format`: Name of the storage format of the target. User-specified with `tar_target()` or `tar_options()`.
* `iteration`: Iteration mode of the target's value, either `"vector"` or `"list"`. User-specified with `tar_target()` or `tar_options()`.
...
1. The storage `format` (user-specified with `tar_target()` or `tar_options()`) is different from last time.
1. The `iteration` method (user-specified with `tar_target()` or `tar_options()`) is different from last time.
