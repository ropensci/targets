...
A `subpipeline` is not actually a class of its own, it is just a `pipeline` object with only the direct dependencies of a particular target and no values or cache objects in those dependencies. Its only purpose is to efficiently assist with the mechanics of remote retrieval.
...
A sitrep object keeps track of the overall status of a all a pattern's branches as a group. Its helps make it more efficient to keep track of the progress, runtime, and storage size of an entire pattern.
