...
The `targets` R package is a [Make](https://www.gnu.org/software/make/)-like pipeline toolkit for Statistics and data science in R. `targets` makes the analysis reproducible without costly runtime, speeds up analysis with easy-to-configure parallel computing, and removes the need for data micro-management. A fully up-to-date `targets` pipeline is tangible evidence that the output aligns with the code and data, which substantiates trust in the results.
